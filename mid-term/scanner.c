...
Token* readConstString(void){
	...
	token->string = (char*)malloc(sizeof(char)*200);
	int tokenLength = 0;
	...
	while (charCodes[currentChar] != EOF && charCodes[currentChar] == CHAR_LETTER
				|| charCodes[currentChar] == CHAR_DIGIT){
		
		token->string[++tokenLength] = currentChar;
		readChar();
	}
	
	token->string[++tokenLength] = '\0';
